<template>
   <div class="container my-3">
        <div class="row my=3">
            <div class="col-6">
                <button class="btn btn-primary" v-bind:disabled="!store.is_login" @click="$router.push('/question-create')">게시글 작성</button>
            </div>
            <div class="col-6">
                <div class="input-group">
                    <input type="text" class="form-control" v-model="kw">
                    <button class="btn btn-outline-secondary" @click="() => {store.keyword=kw, store.page=0}">찾기</button>
                </div>
            </div>
        </div>
        <br>
    ...
    <ul class="pagination justify-content-center">
        ...
        <button class="page-link" @click="() => {store.page -= 1}">이전</button>
        </li>

        <li v-for="loop_page in total_page" :key="loop_page" class="page-item">
        <!-- {{ loop_page }} -->
        <button v-if="loop_page >= store.page-5 && loop_page <= store.page+5" class="page-link" :class="{ 'active': loop_page-1 === store.page }" @click="() => {store.page = loop_page-1}">{{ loop_page }}</button>
        </li>

        <!-- 다음페이지 -->
        <li v-if="store.page < total_page - 1" class="page-item">
        <button class="page-link" @click="() => {store.page += 1}">다음</button>
        </li>
    </ul>
    <!-- 페이징처리 끝 -->
...
try {
    ...
    (json) => {
        ...
        console.log("current page : ", pg.value);
